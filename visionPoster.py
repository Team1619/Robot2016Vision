...
class VisionPoster:
    ...
    def __init__(self, width, height, focalLength):
        ...
        self.angleOffsetToAlignedStabilizer = Stabilizer(4, 5)
        self.verticalAngleStabilizer = Stabilizer(10, 5)

    def getFrame(self):
        frame = self.camera.readFrame()
        flag, coordinates, centerX, centerY = self.camera.getCoords(False, False, frame)
        if flag:
            try:
                certified, distance, pivotalAngle, angleOffsetToAligned, verticalAngle, distanceToOptimal, angleToOptimal, errors = self.camera.getVertexData(coordinates)
